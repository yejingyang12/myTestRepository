 ...
 */
function recursionJurisdictionMethod(list,code){
	var boo = false;
	if(list){
		for(var i=0;i<list.length;i++){
			if(list[i].resourceCode==code){
				return true;
			}
			var children = list[i].children;
			if(children){
				boo = recursionJurisdictionMethod(children,code);
				if(boo){
					return boo;
				}
			}
...
function getJurisdictionMethod(returnData,code){
  var boo = false;
  var fatherList = returnData.data;
  if(fatherList){
  	boo = recursionJurisdictionMethod(fatherList,code);
  }
  return boo;
//  code="0102010105"
