...
function help(_this,num) {
  ...
  if (!_this.getAttribute("hasClick")){//正常的时候点击
    if(num==0){//box1
    ...
    _this.setAttribute("hasClick","true");
  }else {//点击之后的样式
    if(num==0){//box1
...
function list() {
  $("#h-list-show").toggle();
  $("#h-list-hidden").toggle();
  $("#h-score-list1").toggle();
  $("#h-score-list2").toggle();
}