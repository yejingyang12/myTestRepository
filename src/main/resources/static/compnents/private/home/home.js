(function () {
  var data = {
    ...
    data: [//假数据
      {
        "systemId": "0093f1aa298848f6aa7f4f0d3eb6aa2c",
        "systemName": "创新创业大赛系统平台",//系统名称
        "companyName": "中国石化", //所属单位
        "plateType": "油田板块",//版块
        "appIsInternet": "是",//是否为互联网应用
        "gradingStatus": 1,//
        "examineStatus": 1,//
        "recordStatus": 1,//
        "evaluationStatus": 1,//
        "examinationStatus": 1,
        "infoSysTypeConstruction": "自建",//建设类型
        "sprankLevel": 4,//登保级别
    },
      {
        "systemId": "0093f1aa298848f6aa7f4f0d3eb6aa2c",
        "systemName": "啊中国石油",//系统名称
        "companyName": "中国石化", //所属单位
        "plateType": "大庆油田板块",//版块
        "appIsInternet": "否",//是否为互联网应用
        "gradingStatus": 1,//
        "examineStatus": 1,//
        "recordStatus": 1,//
        "evaluationStatus": 1,//
        "examinationStatus": 1,
        "infoSysTypeConstruction": "统建",//建设类型
        "sprankLevel": 1,//登保级别
      },
      {
        "systemId": "0093f1aa298848f6aa7f4f0d3eb6aa2c",
        "systemName": "和中国石油",//系统名称
        "companyName": "a中国石化", //所属单位
        "plateType": "庆油田板块",//版块
        "appIsInternet": "否",//是否为互联网应用
        "gradingStatus": 1,//
        "examineStatus": 1,//
          ...
          listsort:function () {
            ...
            for (var i = 0; i < imgArrow.length; i++) {
              ...
              imgArrow[i].onclick=function () {
                flagOne*=-1;
                var sysName=[],comName=[], plType=[],appInter=[],constructor=[],level=[];
                //循环data里的data数组
                for (var j = 0; j < data.data.length; j++) {
                  var item = data.data[j];//拿到每个对象
                  //拿到对应的属性值 并且放到每个数组里
                  sysName.push(item.systemName);
                  comName.push(item.companyName);
                  plType.push(item.plateType);
                  appInter.push(item.appIsInternet);
                  constructor.push(item.infoSysTypeConstruction);
                  level.push(item.sprankLevel);
                }
                //对每个数组也就是对应表格的每一列进行排序
                console.log(sysName,comName,plType, appInter ,constructor ,level);
                sysName.sort(function (a, b) {return (a.localeCompare(b))*flagOne});
                comName.sort(function (a, b) {return (a.localeCompare(b))*flagOne});
                plType.sort(function (a, b) {return (a.localeCompare(b))*flagOne});
                appInter.sort(function (a, b) {return (a.localeCompare(b))*flagOne});
                constructor.sort(function (a, b) {return (a.localeCompare(b))*flagOne});
                level.sort(function (a, b) {return (a-b)*flagOne});
                console.log(sysName,comName,plType, appInter ,constructor ,level);
              };
