...
//    response.setContentType(contentType);
  ...
  public static String upload(HttpServletRequest request, String filePath, 
    ...
    if(multipartResolver.isMultipart(request)){
      ...
      //验证文件夹是否存在，不存在则重新创建
      File updateFilePath = new File(filePath);
      ...
      while(iter.hasNext()){
        ...
        if(file != null){
          //取得当前上传文件的文件名称
          String strMyFileName = file.getOriginalFilename();
          //如果名称不为“”,说明该文件存在，否则说明该文件不存在
          if(strMyFileName.trim() !=""){
          	if(fileName!=""){
          		 String strExtensionName = strMyFileName.substring(strMyFileName.lastIndexOf("."));
               filePath = filePath + fileName + strExtensionName;
          	}else{
              filePath = filePath + strMyFileName;
          	}
            //定义上传路径
